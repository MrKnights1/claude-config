...
5. Launch 2 Task subagents IN PARALLEL with `subagent_type: "general-purpose"`. Each agent gets the full file contents of all changed files and the Persona above. Each does a fresh, independent review of the current state of those files — security, performance, design, edge cases, everything. The redundancy is intentional: what one reviewer misses, another will catch.
